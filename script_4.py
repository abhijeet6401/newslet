# Create the newsletter generator module
newsletter_generator_py_content = """import logging
...
<body>
    <div class="newsletter-container">
        ...
        <div class="footer">
            <p><strong>Newsletter Statistics:</strong> {total_articles} articles analyzed from {total_sources} sources</p>
    ...
    def generate_html_newsletter(self, articles, title):
        ...
        summary_section = self.generate_summary_section(articles)
        
        # Generate statistics
        statistics = self.generate_statistics_section(articles)
        
        # Generate content sections for each category
        content_sections = ""
        for category, category_articles in categories.items():
            content_sections += f'<div class="category-section">'
            content_sections += f'<h2 class="category-header">{category} ({len(category_articles)} articles)</h2>'
            
            for article in category_articles:
                content_sections += self.generate_article_html(article)
    ...
    def generate_summary_section(self, articles):
        ...
        <div class="summary">
            ...
            <p>This newsletter covers <strong>{len(articles)} articles</strong> from multiple financial news sources. 
            The overall market sentiment appears <strong>{sentiment_text}</strong> with an average sentiment score of <strong>{avg_sentiment:.2f}</strong>.</p>
            
            <p><strong>{high_importance_count} high-priority articles</strong> require immediate attention.</p>
        ...
        for article in top_articles:
            summary += f'<li><strong>{article.get("title", "")}</strong> (Importance: {article.get("importance_score", 0):.1f}/1.0)</li>'
...
with open('newsletter_generator.py', 'w', encoding='utf-8') as f:
    f.write(newsletter_generator_py_content)

print("✅ Created newsletter_generator.py - Newsletter generation module")
print("File size:", len(newsletter_generator_py_content), "characters")